...
from monai.networks.nets.sam import TwoWayTransformer
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.downsample_layer = UnetrBasicBlock(
            spatial_dims=spatial_dims,
        ...
        self.skip_layer = nn.Sequential(
            nn.Conv3d(in_channels=in_channels, out_channels=feature_size//2, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(feature_size//2),
            nn.ReLU(inplace=True)
        )
        """nn.Sequential(
            nn.Conv3d(in_channels=in_channels, out_channels=feature_size//2, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm3d(feature_size//2),
            nn.ReLU(inplace=True)
        )"""
        self.upsample_layer = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size//2, out_channels=out_channels)
        '''nn.Sequential(
            nn.ConvTranspose3d(in_channels=feature_size//2, out_channels=out_channels, kernel_size=3, stride=2, padding=1, output_padding=1),
        ...
        #UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size, out_channels=feature_size//2)
        

        self.txt_embed_dim = 4096
        feature_size_list = [
            feature_size,         # encoder1의 out_channels
            2 * feature_size,         # encoder2의 out_channels, 동일한 feature_size 사용
            4 * feature_size,     # encoder3의 out_channels
            8 * feature_size,     # encoder4의 out_channels
            16 * feature_size      # encoder10의 out_channels
        ]
    ...
    def interactive_alignment(self, hidden_states_out, text):

        #text2vision projection
        report_l = []
        for i in self.txt2vis._modules.keys():
            #print(text, type(text))
            report_l.append(self.txt2vis._modules[i](text))
            #print(self.txt2vis._modules[i](text).shape)
    ...
    def forward(self, x_in, text=None):
        x_downsampled = self.downsample_layer(x_in)
        x_skip = self.skip_layer(x_in)
        if not torch.jit.is_scripting():
            self._check_input_size(x_downsampled.shape[2:])

        hidden_states_out = self.swinViT(x_downsampled, self.normalize)
        if text is not None:
            hidden_states_out=self.interactive_alignment(hidden_states_out, text)
        
        enc0 = self.encoder1(x_downsampled)
        ...
        #print(f"dec0 : {dec0.shape}, enc0: {enc0.shape}, dec1 : {dec1.shape}")
        logits = self.out(out, x_skip)
        #print(f"logits : {logits.shape}, out: {out.shape}, x_downsampled : {x_downsampled.shape}")
        logits_upsampled = self.upsample_layer(logits)
